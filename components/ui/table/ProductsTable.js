...
const ProductsTable = async () => {
	...
	return (
		<table className="w-full text-xs text-left text-black bg-gray-50 rounded">
			...
			<tbody className="w-full text-xs text-left">
				{products.map((product) => (
					<tr key={product.slug}>
						<td className="px-5 py-2 w-3/12 uppercase">
							{product.title}
						</td>
						<td className="px-3 py-2 w-1/12">${product.price}</td>
						<td className="px-3 py-2 w-1/12">{product.stock}</td>
						<td className="px-3 py-2 w-2/12">{product.category}</td>
						...
						<td className="pr-3">
							<div className="flex gap-2">
								...
								<Link href={`/admin/delete/${product.slug}`}>
									<ButtonWithIcon
